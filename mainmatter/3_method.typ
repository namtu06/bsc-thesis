...
Although, CPTAC does provide RNA-seq data for "healthy" samples, they are Normal Adjacent Tissue (NAT) which are brain tissues that have yet to exhibit cancerous features nearby the extracted tumors @cptac3_gdc @liu2024_cptac_codex. Aran et al. @aran2017 showed NAT samples specifically from CPTAC's cancer samples have substantial transcriptomic and epigenetic alterations, causing them to be in a molecular "unique intermediate state" between healthy and tumor, despite not yet exhibiting cancerous features. Therefore, NAT samples provided by CPTAC were considered unsuitable as a baseline for comparison in this thesis. In its place, the Genotype-Tissue Expression (GTEx) project provides comprehensive, and peer-reviewed RNA-seq data from healthy brain tissue, suitable for comparison in cancer research @gtex2020 @zeng2019.
